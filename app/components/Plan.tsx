...
const Plan = ({icon: MainIcon, price, name, slogan, option1, option2, option3, option4, buttonIcon:ButtonIcon, type, buttonLink}:PlanProps) => {
  return (
    <div className='relative flex flex-col items-center gap-2'>
        ...
        <p className={`w-fit text-center text-xs font-semibold uppercase rounded-full border-[1.5px] px-3 py-1 ${type === 'limited' ? 'text-lime-400 border-lime-400 mt-8' : 'text-cyan-500 border-r-cyan-500 mt-4'}`}>{name}</p>
        ...
        {type === 'limited' && (
            <p className='mt-6 text-xs text-lime-400'>-- Limited time offer --</p>
        )}
    </div>
  )
}
