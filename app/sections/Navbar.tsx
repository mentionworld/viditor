...
const NavbarLink = ({href, currentSection, children}:NavbarLinkProps) => {
  ...
  return (
    <Link href={href} className={`hidden relative md:block font-semibold ${isActive ? 'text-lime-400' : ''}`}>
      {children}
      {isActive &&  <div className='absolute bottom-[-3px] w-full h-[3px] bg-lime-400 rounded-full' />}
    </Link>
  )
}
